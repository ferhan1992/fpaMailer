...
public class MainWindowController implements Initializable {
    ...
    private TreeItem<Component> mainDir = new TreeItem<>(new Folder(ROOT_PATH, true), new ImageView(FOLDER_ICON));
        ...
        for (File y : file.listFiles()) {
            if (y.isDirectory()) {
                if (y.list().length == 0) {
                    ...
                    TreeItem<Component> z = new TreeItem<>(new Folder(y.getAbsoluteFile(), true), new ImageView(FOLDER_ICON));
                    x.getChildren().add(z);
                }
            } else {
                TreeItem<Component> z = new TreeItem<>(new FileElement(y.getAbsoluteFile()), new ImageView(FILE_ICON));
                x.getChildren().add(z);
            }
        }
    }
